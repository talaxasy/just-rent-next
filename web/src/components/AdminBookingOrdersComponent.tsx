import {Box, Button, Divider, Flex, Image, StackDivider, Text, VStack} from '@chakra-ui/react';
import moment from 'moment';
import NextLink from 'next/link';
import React from 'react';
import {
  GetAdminBookingsDocument,
  GetAdminBookingsQuery,
...
const AdminBookingOrdersComponent: React.FC<AdminBookingOrdersComponentProps> = ({
  data,
  finished,
}) => {
  ...
  return (
    <VStack
      ...
      align="stretch"
    >
      {data?.getAdminBookings && data?.getAdminBookings.length !== 0 ? (
        data.getAdminBookings.map(el => (
          <Flex w="100%" height="auto" key={`${el.id}_${el.createdAt}`}>
              ...
              <NextLink href="/house/[id]" as={`/house/${el.house.id}`}>
                ...
                >
                  <Image
                    ...
                    h="100%"
                    objectFit="cover"
                    src={`http://localhost:4000/images/${el.house.user.id}/${el.house.pictureUrl[0]}`}
                  />
                </Box>
              </NextLink>
            ...
            <Flex w="350px">
              <Box fontSize="18px">
                <Text>
                  Бронь: с {moment(el.startDate, 'x').format('DD.MM.YY')} по{' '}
                  {moment(el.endDate, 'x').format('DD.MM.YY')}
                </Text>
                <Text>
                  Статус:{' '}
                  <Text
                    display="inline-block"
                    color={el.status === 'в ожидании' ? '#ff8d00' : '#26bf00'}
                  >
                    {el.status}
                  </Text>
                </Text>
                <Text>
                  Хозяин:{' '}
                  {el.house.user.firstName ? (
                    <Text display="inline-block" fontWeight="500">
                      {el.house.user.firstName} {el.house.user.secondName}
                    </Text>
                  ) : (
                    <Text display="inline-block" opacity=".7">
                      Неизвестно
                    </Text>
                  )}
                </Text>
                ...
                <Text>
                  Цена: <b>BYN {el.house.price} / ночь</b>
                </Text>
              </Box>
            </Flex>
            <Divider
            ...
            <Box fontWeight="600" w={finished ? '540px' : '400px'}>
              <Flex justifyContent="space-between" my="7px">
                <Box>{`BYN ${el.house.price} x ${datediff(
                  moment(el.startDate, 'x'),
                  moment(el.endDate, 'x'),
                )} ночей`}</Box>
                <Box>{`BYN ${
                  el.house.price * datediff(moment(el.startDate, 'x'), moment(el.endDate, 'x'))
                }`}</Box>
              </Flex>
              <Flex justifyContent="space-between" my="7px">
              ...
              <Flex justifyContent="space-between" my="7px">
                ...
                <Box>{`BYN ${Math.round(
                  (14 / 100) *
                    (el.house.price *
                      datediff(moment(el.startDate, 'x'), moment(el.endDate, 'x'))) +
                    el.house.price * datediff(moment(el.startDate, 'x'), moment(el.endDate, 'x')),
                )}`}</Box>
              </Flex>
            </Box>
        ...
        <Box fontSize="lg" opacity=".7">
          {finished
            ? 'У вас пока нет истории заказов жилья'
            : 'У вас пока нет заказов брони на жильё'}
        </Box>
      )}
    </VStack>
  );
