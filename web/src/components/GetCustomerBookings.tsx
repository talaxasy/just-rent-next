...
const GetCustomerBookings: React.FC<GetCustomerBookingsProps> = ({
  getCustomerBookings,
  finished,
}) => {
  ...
  return (
    ...
    >
      ...
      getCustomerBookings?.getCustomerBookings.length !== 0 ? (
        getCustomerBookings.getCustomerBookings.map(el => (
        ...
        <Box fontSize="lg" opacity=".7">
          {finished ? 'У вас пока нет истории бронирования' : 'У вас пока нет арендованного жилья'}
        </Box>
      )}
    </VStack>
  );
