...
const MapWithAMarker = withGoogleMap<{
  ...
  return (
    <GoogleMap defaultZoom={17} defaultCenter={props.defaultCenter}>
...
const House: React.FC<HouseProps> = ({}) => {
  ...
  const {data: bookingData} = useGetBookingsQuery({
    variables: {
      houseId: data?.house ? data.house.id : 0,
    },
  });
  ...
  if (!data?.house) return <Layout>Такого дома не существует!</Layout>;

  const datediff = (first: moment.Moment, second: moment.Moment) => {
    return Math.round(
      (parseInt('' + second, 10) - parseInt('' + first, 10)) / (1000 * 60 * 60 * 24),
    );
  };
  console.log('wdwdwd');
        ...
        <Flex flexDir="column" mt="20px" mb="50px">
          <Box>
            ...
            <Box display="flex" alignItems="center" mb="5px">
              ...
              {new Date() >
              new Date(
                new Date(parseInt(data.house.createdAt)).setDate(
                  new Date(parseInt(data.house.createdAt)).getDate() + 7,
                ),
              ) ? null : (
                <Box fontSize="26px" mx="10px">
                  ·
                </Box>
              )}
              {new Date() >
              new Date(
                ...
                <Badge borderRadius="full" px="2" colorScheme="airbnb">
                  Новый
                </Badge>
              )}
              <Box fontSize="26px" mx="10px">
                ...
                return (
                  ...
                  >
                    <Image
                      ...
                      h="100%"
                      objectFit="cover"
                      src={`http://localhost:4000/images/${data.house?.userId}/${pic}`}
                    />
                  </Box>
                );
              })}
              ...
              <Flex>
                <Flex flexDir="column" flex="0 1 100%">
                  <Text fontSize="2xl" fontWeight="600">
                    {data.house.house_type.name}, хозяин: {data.house.user.username}
                  </Text>
                  <Flex fontSize="md" fontWeight="400" alignItems="center">
                    ...
                    <Box fontSize="26px" mx="5px">
                      ·
                    </Box>
                    <Box>
                      {data.house.bedroom_count > 0 && `${data.house.bedroom_count} спальни`}
                    </Box>
                    {data.house.bedroom_count > 0 && (
                      <Box fontSize="26px" mx="5px">
                        ·
                      </Box>
                    )}
                  ...
                  >
                      ...
                      onSubmit={async values => {
                        if (typeof values.guests_count !== 'string') {
                          const {data: myData} = await doBooking({
                            ...
                            refetchQueries: [
                              {
                                query: GetCustomerBookingsDocument,
                              },
                            ],
                          });
                          if (myData?.doBooking) {
                    ...
                    >
                      {({isSubmitting, setValues, values}) => (
                        <Form>
                          <Box fontWeight="600px" mb="20px" fontSize="22px">
                            <Text
                              display="inline-block"
                              fontWeight="700"
                            >{`BYN ${data.house?.price}`}</Text>
                            {` / ночь`}
                          </Box>
                          <Box
                            mb="20px"
                          ...
                          {startDate && endDate && (
                            <Box fontWeight="600">
                              <Flex justifyContent="space-between" my="7px">
                                <Box>{`BYN ${data.house!.price} x ${datediff(
                                  startDate,
                                  endDate,
                                )} ночей`}</Box>
                                <Box>{`BYN ${
                                  data.house!.price * datediff(startDate, endDate)
                                }`}</Box>
                              </Flex>
                              <Flex justifyContent="space-between" my="7px">
                              ...
                              <Flex justifyContent="space-between" my="7px">
                                <Box>Итого</Box>
                                <Box>{`BYN ${Math.round(
                                  (14 / 100) * (data.house!.price * datediff(startDate, endDate)) +
                                    data.house!.price * datediff(startDate, endDate),
                                )}`}</Box>
                              </Flex>
                            </Box>
            ...
            {data.house.latitude && data.house.longitude && (
              <>
                ...
                <MapWithAMarker
                  ...
                  defaultCenter={{
                    lat: data.house.latitude,
                    lng: data.house.longitude,
                  }}
                  markerPosition={{
                    lat: data.house.latitude,
                    lng: data.house.longitude,
