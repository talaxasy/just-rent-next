...
import { getConnection, LessThan, LessThanOrEqual, MoreThanOrEqual, Not } from 'typeorm';
...
export class HouseResolver {
    ...
    async searchListings(
        ...
        let listing = getConnection()
            .getRepository(House)
            .createQueryBuilder('h');

        let step1: null | House[] = null;
        if (city && state && street && country && guests_count) {
            step1 = await House.find({ where: { city, state, street, country, guests_count: MoreThanOrEqual(guests_count) } });
        }

        let step2: null | House[] = null;
        if (city && state && street && guests_count) {
            step2 = await House.find({ where: { city, state, street, guests_count: MoreThanOrEqual(guests_count) } });
        }
        ...
        if (street && state && guests_count) {
            step4 = await House.find({ where: { street, state, guests_count: MoreThanOrEqual(guests_count) } });
        }
